...
const ServiceCard = ({ icon: Icon, title, description, index }) => (
  <motion.div
    ...
    className="group relative rounded-2xl border border-zinc-900/5 bg-zinc-50 p-7 md:p-8 hover:border-zinc-900/15 transition-colors duration-500 overflow-hidden"
  >
    ...
    <div className="relative">
      <span className="inline-flex h-11 w-11 items-center justify-center rounded-xl border border-zinc-900/10 bg-zinc-100">
        <Icon size={18} className="text-zinc-900/85" />
      </span>
      <h3 className="font-heading mt-7 text-xl tracking-tight font-medium">
        {title}
      </h3>
      <p className="mt-3 text-sm text-zinc-900/55 leading-relaxed">{description}</p>
      <div className="mt-7 text-[10px] uppercase tracking-[0.24em] text-zinc-900/30">
        0{index + 1}
      </div>
    </div>
  </motion.div>
